...
const NavbarUser = ({ setIsLoggedIn }) => {
  ...
  const handleReject = (id) => {
    // socket.emit("rejectFriendRequest", id);
    setFriendRequests(friendRequests.filter((request) => request.id !== id));
  };
  ...
  return (
    <nav className="bg-gradient-to-r from-blue-600 to-purple-600 text-white p-4 shadow-lg">
      <div className="container mx-auto flex justify-between items-center">
        ...
        <div className="relative mx-4 flex-1 max-w-xl hidden md:block" ref={searchRef}>
          ...
          {showSearchResults && searchResults?.length > 0 && (
            <div className="absolute mt-2 w-full bg-white rounded-lg shadow-lg py-2 text-gray-800 z-50">
              {searchResults.map((result) => (
                <div key={result._id} className="px-4 py-2 hover:bg-gray-100 cursor-pointer">
                  <div className="flex items-center">
                    <User size={16} className="mr-2 text-blue-600" />
                    <span>{result.name}</span>
                    <button onClick={()=>request_friend(result._id)} className= "ml-auto  text-white bg-green-700 hover:bg-green-800 focus:outline-none focus:ring-4 focus:ring-green-300 font-medium rounded-full text-sm px-5 py-2.5 text-center me-2 mb-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800">Green</button>
                  </div>
                </div>
              ))}
            </div>
          ...
          {showSearchResults && (searchResults === null || searchResults?.length === 0) && (
            <div className="absolute mt-2 w-full bg-white rounded-lg shadow-lg py-2 text-gray-800 z-50">
        ...
        <ul className={`md:flex md:space-x-6 absolute md:static top-16 left-0 w-full bg-gradient-to-r from-blue-600 to-purple-600 md:w-auto md:bg-transparent md:flex-row flex-col items-center transition-all duration-300 ease-in-out ${isOpen ? "block" : "hidden"
          }`}>
          <li>
            <Link to="/" className="flex items-center px-4 py-2 md:px-0 hover:text-purple-200 transition-colors duration-300">
              🏠 <span className="ml-2">Home</span>
            </Link>
          </li>
          <li>
            <Link to="/chat" className="flex items-center px-4 py-2 md:px-0 hover:text-purple-200 transition-colors duration-300">
              💬 <span className="ml-2">Chats</span>
            </Link>
          </li>
          <li>
            <Link to="/status" className="flex items-center px-4 py-2 md:px-0 hover:text-purple-200 transition-colors duration-300">
              ...
              <div className="absolute right-0 top-full mt-2 w-64 bg-white text-black shadow-lg rounded-lg py-2 z-10">
                ...
                {friendRequests.length > 0 ? (
                  friendRequests.map((request) => (
                    <div key={request.id} className="px-4 py-2 hover:bg-gray-50 transition-colors duration-300">
        ...
        <div className="relative">
          ...
          {profileOpen && (
            <div className="absolute right-0 mt-2 w-48 bg-white text-black shadow-lg rounded-lg py-2 z-10">
              <Link to="/profile" className="flex items-center px-4 py-2 hover:bg-gray-100 transition-colors duration-300">
                👤 <span className="ml-2">Profile</span>
              </Link>
              <Link to="/settings" className="flex items-center px-4 py-2 hover:bg-gray-100 transition-colors duration-300">
                ⚙️ <span className="ml-2">Settings</span>
              </Link>
              <Link to="/contacts" className="flex items-center px-4 py-2 hover:bg-gray-100 transition-colors duration-300">
                📞 <span className="ml-2">Contacts</span>
              </Link>
              <div className="border-t border-gray-200 my-1"></div>
              <Link onClick={() => LogOut_User()} className="flex items-center px-4 py-2 hover:bg-gray-100 transition-colors duration-300 text-red-600">
